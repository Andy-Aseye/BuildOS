/**
 * Comprehensive seed script — populates every table with realistic construction data.
 *
 * Uses Prisma directly for speed and to reach tables without POST endpoints (diary, attendance, files).
 * Also calls /auth/register once to create a valid Supabase user + tenant.
 *
 * Usage:
 *   npx tsx scripts/seed.ts
 *
 * Prerequisites:
 *   - API running at http://localhost:3001 (for the register call)
 *   - Database accessible via apps/api/.env
 */
...
async function seed() {
  ...
  // 1. Register via API to get a real Supabase user
  console.log('1/12 Registering test account via API...');
  const ts = Date.now();
  const email = `demo+${ts}@buildwit.io`;
  const password = 'Demo1234!';
  const companyName = `Asante Construction ${ts}`;

  const res = await fetch(`${API}/auth/register`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ email, password, name: 'Kwame Asante', companyName }),
  });
  if (!res.ok) {
    const err = await res.text();
    throw new Error(`Register failed: ${err}`);
  }
  const { tenant, user: owner } = await res.json();
  ...
  // 2. Create additional users (these are WhatsApp/internal users, no Supabase auth)
  console.log('2/12 Creating team members...');
  const memberDefs = [
    { name: 'Ama Mensah', role: 'PROJECT_MANAGER' as const, email: `ama.m+${ts}@buildwit.io`, phone: `+23324100${ts.toString().slice(-4)}` },
    { name: 'Kofi Boateng', role: 'FOREMAN' as const, phone: `+23324200${ts.toString().slice(-4)}` },
    { name: 'Yaa Osei', role: 'ARCHITECT' as const, email: `yaa.o+${ts}@buildwit.io`, phone: `+23324300${ts.toString().slice(-4)}` },
    ...
    { name: 'Daniel Ankrah', role: 'ARCHITECT' as const, email: `daniel.a+${ts}@buildwit.io`, phone: `+23324800${ts.toString().slice(-4)}` },
  ];

  const users: { id: string; name: string; role: string }[] = [
    { id: ownerId, name: 'Kwame Asante', role: 'OWNER' },
  ];
  ...
  // 3. Create projects
  console.log('3/12 Creating projects...');
  const projectDefs = [
    {
    ...
    },
    {
      ...
      description: 'Public library and community learning center. Two-story structure with reading rooms, computer lab, and auditorium.',
      budgetGhs: 950000,
  ...
  let codeIdx = 1;
  const codePrefix = `P${ts.toString().slice(-4)}`;

  for (const def of projectDefs) {
    const code = `${codePrefix}-${String(codeIdx++).padStart(2, '0')}`;
  ...
  console.log('4/12 Adding project members...');
  for (const project of activeProjects) {
    ...
    const memberSet = [{ id: ownerId, role: 'OWNER' as const }, ...memberPool.filter(u => u.id !== ownerId).map(u => ({ id: u.id, role: u.role as any }))];
    ...
    console.log(`   ✓ ${memberSet.length} members for ${project.code}`);
  }
  console.log();

  // 5. Create phases for each active project
  console.log('5/12 Creating project phases...');
    ...
    for (const pt of phaseTemplates) {
      ...
      await prisma.projectPhase.create({
        data: {
          ...
          name: pt.name,
          order: pt.order,
          percentComplete: pt.pct,
          status: pt.status,
          plannedStart: daysAgo(120 - pt.order * 25 + off),
          plannedEnd: daysAgo(95 - pt.order * 25 + off),
          ...(pt.status === 'COMPLETED' ? {
            actualStart: daysAgo(122 - pt.order * 25 + off),
            actualEnd: daysAgo(97 - pt.order * 25 + off),
          } : {}),
          ...(pt.status === 'IN_PROGRESS' ? {
            actualStart: daysAgo(100 - pt.order * 25 + off),
          } : {}),
        },
      });
    }
    console.log(`   ✓ 8 phases for ${project.code}`);
  }
  console.log();

  // 6. Create cost entries
  console.log('6/12 Creating cost entries...');
  ...
  const costDescs: Record<string, string[]> = {
    MATERIALS: ['Cement (50 bags)', 'Reinforcement steel 16mm', 'Sand & gravel delivery', 'Roofing sheets', 'Electrical cable 100m', 'PVC pipes and fittings', 'Paint (exterior)', 'Ceramic floor tiles', 'Granite cladding panels', 'Timber formwork'],
  ...
  for (const project of activeProjects) {
    const numCosts = randomInt(12, 22);
    for (let i = 0; i < numCosts; i++) {
      const cat = pick([...costCategories]);
      const descs = costDescs[cat] ?? ['General expense'];
    ...
    console.log(`   ✓ ${numCosts} cost entries for ${project.code}`);
  }
  console.log();

  // 7. Create daily logs (diary entries)
  console.log('7/12 Creating diary entries...');
  const diaryActivities = [
    'Block work continued on 2nd floor east wing',
    'Column casting completed for grid C-D',
    'Plumbing rough-in for ground floor bathrooms',
    'Electrical conduit installation in ceiling void',
    'Roof truss installation — 60% complete',
    'Plastering external walls — north facade',
    'Floor tiling in common areas',
    'Window frame installation — aluminum',
    'Steel reinforcement for beam B4-B7',
    'Concrete curing — 1st floor slab',
    'Formwork stripping and cleanup',
    'Waterproofing membrane applied to basement',
    'Site clearing for external works',
  ...
  const diaryRaw = [
    ...
    'Applied waterproofing membrane in basement. Two coats done. Third coat tomorrow. No leaks detected in test area.',
    'Site clearing for car park and landscaping areas. Topsoil stockpiled for later use.',
    'Started boundary wall at main entrance. Foundation trench dug and blinding concrete poured.',
    ...
    'Material delivery: 200 bags cement, 50 lengths rebar 16mm. Stored in covered area. Inventory updated.',
  ];
  ...
  const aiSummaries = [
    'Block work continued on 2nd floor east wing with 12 workers. 30 bags of cement consumed. Clear weather. No incidents.',
    'Concrete pour completed for columns C4-C8 despite late pump arrival. 15 workers on site. No safety issues.',
    'Plumbing rough-in finished for ground floor. Awaiting inspector approval before wall closure. Good productivity.',
    'Electrical conduit work in ceiling void. Material shortage on 20mm conduit — procurement notified. Progress at 70%.',
    'Roof truss installation reached 60%. Crane utilized full day. Estimated completion by midweek.',
    'External plastering on north facade. Favorable drying conditions. Scaffolding repositioned mid-day.',
    'Floor tiling ongoing — lobby and corridors. Minor tile color variation flagged to supplier. Otherwise on track.',
    'Window installation started. 20 of 48 aluminum frames installed on ground floor. Delivery complete.',
    'Steel reinforcement complete for beams B4-B7. Ready for next-day casting. No rework needed.',
  ...
  for (const project of activeProjects) {
    ...
    for (let i = 0; i < numLogs; i++) {
      ...
      // Add 0-3 photos per diary entry
    ...
    console.log(`   ✓ ${numLogs} diary entries for ${project.code}`);
  }
  console.log();

  // 8. Create attendance logs
  console.log('8/12 Creating attendance logs...');
  for (const project of activeProjects) {
    for (let day = 0; day < 30; day++) {
      if (Math.random() < 0.15) continue; // skip some days (weekends/holidays)
      const logDate = daysAgo(day);
      const reporter = pick([...foremanUsers, ...pmUsers]);
    ...
    console.log(`   ✓ ~25 attendance logs for ${project.code}`);
  }
  console.log();

  // 9. Create RFIs
  console.log('9/12 Creating RFIs...');
  const rfiTitles = [
    'Column C4 reinforcement detail clarification',
    'Window dimensions on elevation sheet A-201',
    'Slab penetration locations for MEP',
    'Fire escape route width confirmation',
    'Foundation depth at grid line F',
    'Waterproofing spec for basement walls',
    'Ceiling height in reception area',
    'Staircase handrail material specification',
    'Car park level floor finish',
    'Roof drainage layout confirmation',
    'External cladding color approval',
    'Electrical panel room ventilation requirements',
    'Lift shaft dimensions confirmation',
    'Kitchen exhaust duct routing',
    'Retaining wall design for sloped section',
    'Glass balustrade loading specification',
    'Acoustic insulation for conference rooms',
    'Generator room exhaust clearance',
  ];

  for (const project of activeProjects) {
    const numRfis = randomInt(5, 9);
    let refNo = 1;
    for (let i = 0; i < numRfis; i++) {
      ...
      await prisma.rFI.create({
        data: {
          projectId: project.id,
          tenantId,
          referenceNo: `RFI-${String(refNo++).padStart(3, '0')}`,
          title,
          description: `Clarification required on ${title.toLowerCase()}. Please review the attached drawings and provide guidance as soon as possible.`,
          status,
          ...
          ...(status === 'ANSWERED' || status === 'CLOSED' ? {
            response: 'Reviewed and confirmed per discussion with lead consultant. Proceed as indicated on revised drawing Rev C.',
            respondedAt: daysAgo(randomInt(0, 5)),
          } : {}),
          ...(status === 'CLOSED' ? { closedAt: daysAgo(randomInt(0, 3)) } : {}),
        },
      });
    }
    console.log(`   ✓ ${numRfis} RFIs for ${project.code}`);
  }
  console.log();

  // 10. Create delay logs
  console.log('10/12 Creating delay logs...');
  const delayCauses = ['WEATHER', 'MATERIALS', 'LABOUR', 'DESIGN', 'CLIENT', 'UTILITIES', 'OTHER'] as const;
  const delayDescs: Record<string, string[]> = {
    WEATHER: ['Heavy rainfall halted concrete pour', 'Thunderstorm — unsafe crane operation', 'Flooding on access road', 'Strong winds — suspended work at height'],
    ...
    DESIGN: ['Revised structural drawing pending', 'Architect reviewing facade change', 'MEP clash resolution pending', 'Waiting on structural engineer approval'],
  ...
  for (const project of activeProjects) {
    const numDelays = randomInt(5, 10);
    for (let i = 0; i < numDelays; i++) {
      const cause = pick([...delayCauses]);
      const descs = delayDescs[cause] ?? ['Unexpected delay'];
      const reporter = pick([...foremanUsers, ...pmUsers]);
    ...
    console.log(`   ✓ ${numDelays} delay logs for ${project.code}`);
  }
  console.log();

  // 11. Create materials requests with line items
  console.log('11/12 Creating materials requests...');
  const materialItems = [
  ...
  for (const project of activeProjects) {
    ...
    for (let i = 0; i < numReqs; i++) {
      ...
      const approver = status === 'APPROVED' || status === 'ORDERED' || status === 'DELIVERED' || status === 'PARTIALLY_DELIVERED'
        ? pick(pmUsers) : null;
      ...
      const req = await prisma.materialsRequest.create({
        data: {
          ...
          currency: 'GHS',
          notes: `Materials for ${pick(['foundation', 'superstructure', 'roofing', 'finishing', 'MEP'])} phase`,
      ...
      for (const it of items) {
        ...
        await prisma.materialsRequestItem.create({
          data: {
            ...
            quantity: qty,
            unit: it.unit,
            estimatedUnitCost: it.cost,
            deliveredQuantity: delivered,
            deliveredAt: delivered ? daysAgo(randomInt(0, 5)) : null,
          },
        });
      }
    }
    console.log(`   ✓ ${numReqs} materials requests for ${project.code}`);
  }
  console.log();

  // 12. Create drawing reviews with revisions
  console.log('12/12 Creating drawing reviews...');
  const drawingTitles = [
    'Architectural Floor Plans — Ground Floor',
    'Architectural Floor Plans — First Floor',
    'Structural Layout — Foundation',
    'Structural Layout — Columns & Beams',
    'MEP Coordination Drawing',
    'External Elevation — North',
    'External Elevation — South',
    'Roof Framing Plan',
    'Landscape & Parking Layout',
  ...
  for (const project of activeProjects) {
    const numDrawings = randomInt(4, 7);
    const titles = pickN(drawingTitles, numDrawings);
    for (const title of titles) {
      ...
      const review = await prisma.drawingReview.create({
        data: {
          ...
          tenantId,
          title,
          description: `${title} — submitted for consultant review and approval.`,
          status,
          submittedById: submitter.id,
    ...
    console.log(`   ✓ ${numDrawings} drawing reviews for ${project.code}`);
  }
  console.log();

  // 13. Create project files
  console.log('Bonus: Creating project files...');
  const fileTemplates = [
  ...
  for (const project of activeProjects) {
    ...
    console.log(`   ✓ ${files.length} files for ${project.code}`);
  }
  console.log();

  // 14. Create progress reports
  console.log('Bonus: Creating progress reports...');
  for (const project of activeProjects) {
    ...
    for (let i = 0; i < numReports; i++) {
      ...
      await prisma.progressReport.create({
        data: {
          ...
          narrativeSummary: `Project ${project.name}: During this period, significant progress was made on the superstructure and MEP rough-in. ${randomInt(12, 25)} workers averaged on site daily. Budget utilization at ${randomInt(30, 75)}% of plan. Key milestones achieved include completion of ${pick(['column casting', 'beam pouring', 'roof truss installation', 'floor slab'])}. Weather caused ${randomInt(1, 4)} days of delay. Overall project is ${pick(['on track', 'slightly behind schedule', 'ahead of schedule'])}.`,
    ...
    console.log(`   ✓ ${numReports} progress reports for ${project.code}`);
  }
  ...
  await prisma.tenant.update({
    ...
    data: {
      primaryColor: '#1e293b',
      logoUrl: 'https://ui-avatars.com/api/?name=AC&background=1e293b&color=fff&size=128',
    },
  });
  ...
  console.log(`    ${activeProjects.length * 8} project phases`);
  console.log(`    ~${activeProjects.length * 17} cost entries`);
  console.log(`    ~${activeProjects.length * 20} diary entries + photos`);
  console.log(`    ~${activeProjects.length * 25} attendance logs`);
  console.log(`    ~${activeProjects.length * 7} RFIs`);
  console.log(`    ~${activeProjects.length * 7} delay logs`);
  console.log(`    ~${activeProjects.length * 4} materials requests`);
  console.log(`    ~${activeProjects.length * 5} drawing reviews`);
  console.log(`    ~${activeProjects.length * 7} project files`);
  console.log(`    ~${activeProjects.length * 3} progress reports`);
  console.log('\n════════════════════════════════════════════');
  console.log(`  Log in → http://localhost:3000/login`);
  console.log('════════════════════════════════════════════\n');
}
